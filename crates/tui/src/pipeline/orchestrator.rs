        ...
        let handles: Vec<_> = chunk
            ...
            .map(|(gen_name, scope)| {
                ...
                std::thread::spawn(move || {
                    ...
                    let outcome = run_container(step, &cancel, &phase_clone, &tx);
...
/// lines as `PipelineEvent::Log`. Writes the buffered log to the step's
/// `log_path` after the container exits.
fn run_container(
    step: DockerStep,
    cancel: &CancelToken,
    phase: &Phase,
